-- Phase 2 analytics refresh: exercise_performance, muscle_weekly_volume, fatigue/deload

create or replace function public.calc_e1rm_simple(p_weight_kg numeric, p_reps int)
returns numeric
language sql
immutable
as $$
  select case
    when p_reps is null or p_reps <= 0 or p_reps > 10 or p_weight_kg is null or p_weight_kg <= 0
      then null
    else round((p_weight_kg * (1 + p_reps / 30.0))::numeric, 1)
  end;
$$;
